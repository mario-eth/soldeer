...
The lockfile records its format in a `version` key at the root of the file. Lockfiles without that key were generated by
Soldeer v0.11 or older, which stripped a wrapping root directory when extracting archives. Registry packages whose
archive uses such a layout are therefore extracted differently by newer versions, and their `integrity` checksum no
longer matches. This is expected and does not indicate that a package was tampered with: the ZIP file itself is still
verified against the `checksum` field, and the first `soldeer install` run refreshes the affected checksums and migrates
the lockfile to version 2. From that point on, a failed integrity check means the installed files really did change.
