...
#[tokio::test]
async fn test_install_migrates_legacy_lockfile() {
    let dir = testdir!();
    let contents = r#"[dependencies]
mylib = "1.1"
"#;
    ...
    // serve the file with mock server, and the registry endpoint which resolves the download URL
    let mut server = mockito::Server::new_async().await;
    server.mock("GET", "/file.zip").with_body_from_file(zip_file).create_async().await;
    mock_revision(&mut server, "mylib", "1.1.0").await;

    // a lockfile generated by soldeer v0.11 or older: no version key, and an integrity checksum
    // which was computed from the old extraction layout
    let lock = format!(
        r#"[[dependencies]]
name = "mylib"
version = "1.1.0"
...
"#,
    ...
    // the stale checksum was refreshed and the lockfile migrated to the current format
    let lockfile = read_lockfile(dir.join(SOLDEER_LOCK)).unwrap();
    assert_eq!(lockfile.version, LockFileVersion::V2);
    assert_eq!(
        lockfile.entries.first().unwrap().as_http().unwrap().integrity,
        "bcc66b553ea004dc1ccf39e9d3cda6487edec8e58446e6f04a1e7c4b2e8535f1"
    );
}

#[tokio::test]
async fn test_install_does_not_migrate_current_lockfile() {
    let dir = testdir!();
    let contents = r#"[dependencies]
mylib = "1.1"
"#;
    ...
    // a lockfile in the current format whose integrity checksum does not match: this is not a
    // migration, so the lockfile must be left alone
    let lock = r#"version = 2
...
"#;
    ...
    .await;
    assert!(res.is_ok(), "{res:?}");
    assert_eq!(fs::read_to_string(dir.join(SOLDEER_LOCK)).unwrap(), lock);
}
