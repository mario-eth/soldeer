...
use cliclack::{
    ...
    multi_progress, outro, outro_cancel,
};
use config::{
...
pub async fn run(command: Subcommands) -> Result<(), SoldeerError> {
    match command {
        Subcommands::Init(init) => {
            ...
            init_command(init).await.map_err(|e| {
        ...
        Subcommands::Install(cmd) => {
            ...
            install_command(cmd).await.map_err(|e| {
...
async fn init_command(cmd: commands::Init) -> Result<(), SoldeerError> {
    if cmd.clean {
...
async fn install_command(cmd: commands::Install) -> Result<(), SoldeerError> {
    let config_path = get_config_path()?;
